...
import cv2
from core.tipos import DegradadorPorPieza, BoolArray, DegradadorGlobal
...
def generar_mascara_completamente_verdadera(alto:int, ancho:int) -> BoolArray:
    mascara_numerica = np.ones((alto, ancho))
    mascara_booleana = (mascara_numerica == 1)
    return mascara_booleana

def aplicar_degradacion_global(imagen: np.ndarray, generator: np.random.Generator, degradacion_global:Optional[DegradadorGlobal]) -> np.ndarray:
    if degradacion_global is not None:
        img_degradada = degradacion_global(imagen, generator)
    else:
        img_degradada = imagen.copy()

    return img_degradada

def cortar_imagen_en_piezas_sin_ranuras(
    imagen: np.ndarray,
    cantidad_filas: int,
    cantidad_columnas: int,
) -> Tuple[List[np.ndarray], List[BoolArray]]:
    ...
    piezas:List[np.ndarray] = []
    mascaras:List[BoolArray] = []

    for r in range(cantidad_filas):
        for c in range(cantidad_columnas):

            y_inicio_pieza = r * alto_pieza
            y_fin_pieza = y_inicio_pieza + alto_pieza

            x_inicio_pieza = c * ancho_pieza
            x_fin_pieza = x_inicio_pieza + ancho_pieza 
            
            pieza = imagen[y_inicio_pieza:y_fin_pieza, x_inicio_pieza:x_fin_pieza].copy()
            mascara= generar_mascara_completamente_verdadera(alto_pieza, ancho_pieza)

            piezas.append(pieza)
            mascaras.append(mascara)
    
    return piezas, mascaras

def cortar_imagen_en_piezas_con_ranuras(imagen:np.ndarray, jigsaw:JigsawGridGeometry, padding = 30, anular_pixel_fuera_mascara=True) ->  Tuple[List[np.ndarray], List[BoolArray]]:
    piezas_ordenadas:List[np.ndarray] = []
    piezas_mascaras:List[BoolArray] = []
    for fila in range(jigsaw.filas):
        for columna in range(jigsaw.columnas):
            pieza_img, mascara, _ = jigsaw.extract_piece_image(imagen, fila, columna, padding=padding, anular_pixeles_fuera_mascara=anular_pixel_fuera_mascara)
            piezas_ordenadas.append(pieza_img)
            piezas_mascaras.append(mascara)

    return piezas_ordenadas, piezas_mascaras

def cortar_piezas(imagen:np.ndarray, filas:int , columnas:int, tiene_ranuras:bool, semilla:int, anular_pixel_fuera_mascara:bool) -> Tuple[List[np.ndarray], List[BoolArray]]:

    if tiene_ranuras:
        altura, ancho = imagen.shape[:2]
        jigsaw = JigsawGridGeometry(filas, columnas, altura, ancho, seed=semilla, discrete=True)
        piezas, mascaras = cortar_imagen_en_piezas_con_ranuras(imagen, jigsaw, anular_pixel_fuera_mascara=anular_pixel_fuera_mascara)   
    else:
        piezas, mascaras = cortar_imagen_en_piezas_sin_ranuras(imagen, filas, columnas)     

    return piezas, mascaras

def aplicar_degradaciones_por_piezas(piezas:List[np.ndarray], mascaras:List[BoolArray], degradaciones_por_pieza:List[DegradadorPorPieza], rng: np.random.Generator):

    piezas_actuales = piezas
    mascaras_actuales = mascaras

    for degradacion in degradaciones_por_pieza: 
        piezas_degradadas = []
        nuevas_mascaras = []
        for idx, (pieza, mascara) in enumerate(zip(piezas_actuales, mascaras_actuales)):
        ...
        piezas_actuales = piezas_degradadas
        mascaras_actuales = nuevas_mascaras

    return piezas_actuales, mascaras_actuales

def barajar_piezas(
    piezas_ordenadas: List[np.ndarray],
    mascaras_ordenadas: List[np.ndarray],
    generador: np.random.Generator,
) -> Tuple[List[np.ndarray], List[BoolArray], List[int]]:

    total_piezas = len(piezas_ordenadas)
    permutacion = list(generador.permutation(total_piezas))
    piezas_barajadas = [piezas_ordenadas[i] for i in permutacion]
    mascaras_ordenadas_barajadas = [mascaras_ordenadas[i] for i in permutacion]

    return piezas_barajadas, mascaras_ordenadas_barajadas, permutacion

def generar_diccionario_de_posiciones_reales(
    orden: list[int],
    cantidad_filas: int,
    cantidad_columnas:int
):
    posicion_real_en_rompecabezas = {}
    for idx_nuevo, idx_original in enumerate(orden):
        posicion_real_en_rompecabezas[idx_nuevo] = np.unravel_index(idx_original, (cantidad_filas, cantidad_columnas))

    return posicion_real_en_rompecabezas

def ordenar_piezas(piezas:List[np.ndarray], 
                   mascaras:List[BoolArray], 
                   cantidad_filas:int, 
                   cantidad_columnas:int, 
                   barajar:bool, rng: np.random.Generator) -> Tuple[Dict[int, Tuple[int,int]], List[np.ndarray], List[BoolArray]]:

    if barajar:
        piezas_ordenadas, mascaras_ordenadas, orden = barajar_piezas(piezas, mascaras, rng)
    else:
        piezas_ordenadas = piezas
        mascaras_ordenadas = mascaras
        orden = list(range(len(piezas)))

    diccionario_de_posiciones_reales = generar_diccionario_de_posiciones_reales(orden, cantidad_filas, cantidad_columnas)

    return diccionario_de_posiciones_reales, piezas_ordenadas, mascaras_ordenadas
...
def armar_caso_rompecabezas(
    ...
    degradacion_por_pieza: Optional[DegradadorPorPieza | list[DegradadorPorPieza]] = None,
    semilla: int = 42,
    tiene_ranuras: bool = False,
    metadatos_adicionales: Optional[Dict[str, Any]] = None,
    barajar: bool = True,
    anular_pixel_fuera_mascara = True
) -> Rompecabezas:
    ...
    img_degradada = aplicar_degradacion_global(img_ajustada, rng, degradacion_global)

    piezas_ordenadas, piezas_mascaras = cortar_piezas(img_degradada, cantidad_filas, cantidad_columnas, tiene_ranuras=tiene_ranuras, semilla=semilla, anular_pixel_fuera_mascara=anular_pixel_fuera_mascara)
    ...
    if degradacion_por_pieza is not None:
        ...
        piezas_ordenadas, piezas_mascaras = aplicar_degradaciones_por_piezas(piezas_ordenadas, piezas_mascaras, degradacion_por_pieza, rng)
    ...
    diccionario_de_posiciones_reales, piezas_reordenadas, mascaras_reordenadas = ordenar_piezas(piezas_ordenadas, piezas_mascaras, cantidad_filas, cantidad_columnas, barajar, rng)                          
    ...
    return Rompecabezas(
        piezas=piezas_reordenadas,
        cantidad_filas=cantidad_filas,
        cantidad_columnas=cantidad_columnas,
        posicion_real=diccionario_de_posiciones_reales,
